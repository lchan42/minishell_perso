...
/*   Updated: 2022/07/25 17:09:13 by slahlou          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*__is_expandable(char *str)
{
	...
	if (quote_flag1 == 0 || quote_flag1 == '\"')
		quote_flag1 ^= (*str == '\"') * '\"';
	if (*str == '$'
		&& (ft_isalnum((*(str + 1))) ||  ft_strchr_b("_\"\'", *(str + 1)))//*(str + 1) == '_' || *(str + 1) == '\"' || *(str + 1) == '\'')
		&& *(str + 1) && quote_flag1 != '\'')
...
//test'$USER' $"USER" $'USER' "$USER" $PATH "'$USER'"
// erreur sur :
// $'USER'
// "'$USER'"
...
char	*__expand_string(char *str, char **env)
{
	...
	while (start)
	{
		...
		if (start)
		{
			...
			//printf("expand = %s\n", expand);
			start = __add_exp_to_str(str, start, end, expand);
			str = start;
		}
	}
	return (str);
