...
#include "minishell.h"
// static t_lexer_token *lexer_add_nod (t_llist **lexer_lst, t_lexer_token *tmp_nod)
// {
...
//  *
//  * @param l_data contains t_llist->lexer and t_llist->read_lst
...
/*********************
 * give the right start address, end address and set the type.
 * set the length;
 * return : non 0 value if problem, 0 if ok.
 */
// static int lexer_set_tmp_nod(t_lexer_token *tmp_nod)
...
void	lexer_make(t_lexer_data *l_data, char *str)
{
	...
	while (*str)
	{
		current_nod = (lexer_add_nod(&l_data->lexer, str));
		if (!current_nod)
			lexer_error(ERR_MALLOC_FAIL, current_nod);
